...
class LocalDocumentIndex {
  ...
  async beginUpdate() {
    ...
    if (!this._newCatalog) this._newCatalog = { ...this._catalog };
  }
  ...
  async createIndex(config = { deleteIfExists: false, version: 1 }) {
    ...
    await this._saveIndexData({ items: [], version: config.version });
    await this.loadIndexData();
  }
  ...
  async deleteDocument(uri) {
    ...
    try {
      await Promise.all([
        fs.unlink(path.join(this.folderPath, `${documentId}.txt`)),
        fs.unlink(path.join(this.folderPath, `${documentId}.json`))
      ]);
    } catch {
      // Ignore error
    }
  ...
  getExtension(uri) {
    ...
    if (pos !== -1) {
      return uri.slice(pos + 1).toLowerCase();
    }
    return "";
  }
  ...
  async upsertDocument(uri, text, documentType, metadata) {
    ...
    for (const batch of chunkBatches) {
      ...
      embeddings.push(...response.output);
    }
